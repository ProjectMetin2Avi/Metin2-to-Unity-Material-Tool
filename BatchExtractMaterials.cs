...
public class BatchExtractMaterials : EditorWindow
{
    ...
    [MenuItem("Tools/Batch Extract Materials - @Metin2Avi")]
    private static void Init()
    {
        BatchExtractMaterials window = GetWindow<BatchExtractMaterials>();
        window.titleContent = new GUIContent("Batch Extract Materials - @Metin2Avi");
    ...
    private void DrawModelsToProcessList()
    {
        ...
        GUILayout.BeginHorizontal();
        EditorGUILayout.PrefixLabel("Model Klasörü");

        EditorGUI.BeginChangeCheck();
        selectedFolder = EditorGUILayout.ObjectField(selectedFolder, typeof(DefaultAsset), false) as DefaultAsset;
        if (EditorGUI.EndChangeCheck() && selectedFolder != null)
        {
            string folderPath = AssetDatabase.GetAssetPath(selectedFolder);
            if (AssetDatabase.IsValidFolder(folderPath))
            {
                // Klasördeki tüm model dosyalarýný bul
                ...
                foreach (string guid in modelGuids)
                {
                    ...
                    if (modelAsset != null && PrefabUtility.GetPrefabAssetType(modelAsset) == PrefabAssetType.Model)
                    {
                        modelData.Add(new ExtractData(modelAsset));
                    }
                }
